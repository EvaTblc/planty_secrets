...
menthe = Plant.create!(name: "Menthe", species: "Plante", idapi: "Lamiaceae", infos: "La menthe verte est une plante aromatique vivace qui peut vite devenir envahissante de par sa rapidité à s'étaler.C'est une plante odorante à feuilles ovales et dentées qui peut mesurer jusqu'à 80cm. Les fleurs de la menthe sont blanches, roses, jaunes… selon l’espèce. La menthe demande peu d’entretien et sa rusticité en fait une plante facile.", medecine:"Les menthes cultivées (verte et poivrée) exercent une action sur le système nerveux : elles sont recommandées dans les cas d'insomnie, nervosité, palpitation, migraine, névralgie, vertige. Elles combattent aussi mauvaise haleine et aérophagie.", trouver:"La Menthe verte est la menthe commune d'Europe occidentale qui pousse aussi bien dans les jardins que dans les prairies sauvages, les fossés et les lisières de forêts. Cette plante aromatique est originaire d'une grande zone géographique qui s'étent de l'Asie occidentale à l'Europe et le bassin méditerranéen. Elle s'installe facilement dans tous les types de sol un peu frais, au soleil ou à l'ombre." )
file = File.open(Rails.root.join("db/seeds/images/plants/menthe.jpg"))
menthe.photo.attach(io: file, filename: "menthe.jpg", content_type: "image/jpeg")

puts "Associate user & flowers"

userplants1 = UserPlant.create!(user: chantal, plant: menthe)
...
plantloc3 = PlantLocation.create!(latitude: 47.224167182693506, longitude:-1.5822665824085338, plant: menthe, user_plant: userplants1)
